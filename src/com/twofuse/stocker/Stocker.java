...
import com.twofuse.stocker.Portfolio;
import android.view.animation.Animation;
import com.twofuse.stocker.Rotate3dAnimation;

import android.text.Editable;
import android.text.util.Linkify;
import android.util.Log;

public class Stocker extends ListActivity implements View.OnClickListener {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
	    JSONObject quote;
		AbsoluteLayout cellLayout = new AbsoluteLayout(context);
		int color;
    	cellLayout.setPreferredHeight(35);
    	cellLayout.setPreferredWidth(320);
		cellLayout.setPadding(6, 3, 6, 3);
    	...
    	try {
    		...
    		field.setPadding(6, 0, 6, 3);
    		field.setBackgroundColor(color);
    		field.setText(quote.getString(SYMBOL));
    		AbsoluteLayout.LayoutParams cellLayoutRules = new AbsoluteLayout.LayoutParams(150, 23, 0, 0);
    		cellLayout.addView(field, cellLayoutRules);
    	
    		field = new TextView(context);
    		field.setPadding(6, 0, 6, 3);
    		field.setBackgroundColor(color);
    		field.setText(quote.getString(CURRENT));
    		field.setAlignment(android.text.Layout.Alignment.ALIGN_OPPOSITE);
    		cellLayoutRules = new AbsoluteLayout.LayoutParams(80, 23, 151, 0);
    		cellLayout.addView(field, cellLayoutRules);
    		...
    		cellLayoutRules = new AbsoluteLayout.LayoutParams(50, 23, 250, 0);
    		cellLayout.addView(field, cellLayoutRules);
    	} catch(JSONException e){
			Log.e(TAG, e.getMessage());
    	}
    	if(portfolio.quoteCount() > DISPLAY_COUNT){
    		if(position > DISPLAY_COUNT)
    			stocker.setProgressBarVisibility(false);
    	} else {
    		if(position >= portfolio.quoteCount()-1){
    			stocker.setProgressBarVisibility(false);
    		}
    	}
